...
const RoutePilotAgenda=(()=>{
  ...
  const timeUnitsLabel=order=>CORE.workOrderTimeUnits(order)===2?' · 2 tempos':'';
  ...
  async function init(){
    ...
    if(!globalThis.RoutePilotAuth?.hasCapability('canUseSharedOperations'))return;
    bind();renderTabs();
    ...
    try{await RoutePilotAddressCorrectionsStorage.init();state.addressCorrections=RoutePilotAddressCorrectionsStorage.privateRecords();}catch(error){state.addressCorrections=[];}
  ...
  function render({preserveAgendaScroll=false}={}){
    ...
    if(state.pendingRouteReorganization)root.insertAdjacentHTML('beforeend',renderRouteReorganizationConfirmation());
  ...
  function renderCreateRoute(){
    ...
    return `<section class="route-creation">${dateToolbar('Criar rota')}<div class="route-builder-grid"><div class="route-builder-panel">${renderRouteTechnicianSelection(technicians)}${renderOrderForm()}<section class="work-order-list"><div class="section-title"><div><small>${orders.length} PENDENTES</small><h3>Ordens a distribuir</h3></div></div>${orders.length?orders.map(renderOrderCard).join(''):'<p class="agenda-empty">Nenhuma OS aguardando distribuição para esta data.</p>'}</section><button id="generateRoutesButton" class="agenda-primary" data-agenda-action="generateRoutes" ${orders.length&&selectedCount?'':'disabled'}>${generateLabel}</button>${renderDistribution()}</div><aside class="operations-map-panel"><div class="map-panel-heading"><strong>Distribuição no mapa</strong><span>${state.generated?`${state.generated.allocated} alocadas`:'Aguardando geração'}</span></div><div id="operationsMap" aria-label="Mapa das rotas por técnico"></div></aside></div></section>`;
  }

  /** Monta a seleção e os filtros reutilizáveis da equipe de distribuição. */
  function renderRouteTechnicianSelection(technicians=activeTechnicians()){
    const selected=technicians.filter(item=>state.selected.has(item.id)),selectedNames=selected.length?`${selected.slice(0,3).map(item=>item.name.split(' ')[0]).join(', ')}${selected.length>3?` e mais ${selected.length-3}`:''}`:'Nenhum técnico escolhido',filterOptions=state.routeFilters.map(filter=>`<option value="${esc(filter.id)}" ${filter.id===state.activeRouteFilterId?'selected':''}>${esc(filter.name)}${filter.isDefault?' (padrão)':''}</option>`).join('');
    const filterList=state.routeFilters.length?`<div class="route-filter-list">${state.routeFilters.map(filter=>`<article><div><strong>${esc(filter.name)}</strong><small>${filter.technicianIds.length} técnico${filter.technicianIds.length===1?'':'s'}${filter.isDefault?' · padrão':''}</small></div><button type="button" data-agenda-action="applyRouteFilter" data-id="${esc(filter.id)}">Aplicar</button><button type="button" data-agenda-action="updateRouteFilter" data-id="${esc(filter.id)}">Atualizar</button><button type="button" data-agenda-action="defaultRouteFilter" data-id="${esc(filter.id)}">Padrão</button><button type="button" data-agenda-action="deleteRouteFilter" data-id="${esc(filter.id)}">Excluir</button></article>`).join('')}</div>`:'<p class="route-filter-empty">Nenhum filtro de equipe salvo.</p>';
    const filterManager=`<details class="route-filter-manager"><summary>Salvar ou gerenciar</summary><div class="route-filter-popover"><form id="routeFilterForm"><label>Nome do filtro<input name="name" required maxlength="60" placeholder="Ex.: Equipe Pelotas"></label><label class="route-filter-default"><input type="checkbox" name="isDefault"> Usar como padrão</label><button class="agenda-primary" type="submit">Salvar seleção atual</button></form>${filterList}</div></details>`;
    const body=`<div class="technician-selection-body" ${state.techniciansExpanded?'':'hidden'}><div class="route-filter-toolbar"><label>Filtro da equipe<select id="routeSavedFilter"><option value="">Seleção atual</option>${filterOptions}</select></label>${filterManager}</div><div class="technician-selection-actions"><span>Marque somente quem poderá receber OS nesta rota.</span><button type="button" data-agenda-action="selectAllRouteTechnicians">Selecionar todos</button><button type="button" data-agenda-action="selectNoRouteTechnicians">Nenhum</button></div><div class="technician-chips">${technicians.map(item=>`<label><input type="checkbox" data-agenda-technician="${esc(item.id)}" ${state.selected.has(item.id)?'checked':''}><span>${esc(item.name)}<small>${esc(item.serviceArea||'Sem base definida')}</small></span></label>`).join('')}</div></div>`;
    return `<section class="technician-selection ${state.techniciansExpanded?'':'is-collapsed'}"><div class="section-title"><div><small>EQUIPE DESTA ROTA</small><h3>Escolha os técnicos</h3><span id="routeTechnicianSummary" class="technician-selection-summary">${selected.length} de ${technicians.length} selecionados</span><span id="routeTechnicianNames" class="selected-technician-names">${esc(selectedNames)}</span></div><div><button data-agenda-action="toggleTechnicianNames" aria-expanded="${state.techniciansExpanded}">${state.techniciansExpanded?'Concluir seleção':'Selecionar técnicos'}</button><button data-agenda-action="manageTechnicians">Gerenciar técnicos</button></div></div>${body}</section>`;
  }
  ...
  function updateRouteTechnicianSelection(){
    ...
    const savedFilter=$agenda('routeSavedFilter');if(savedFilter)savedFilter.value=state.activeRouteFilterId||'';
  }

  /** Atualiza somente o seletor de equipe para preservar a OS em preenchimento. */
  function refreshRouteTechnicianSelection(){const current=document.querySelector('.technician-selection');if(current)current.outerHTML=renderRouteTechnicianSelection();updateRouteTechnicianSelection();}
  /** Descarta uma distribuição antiga quando a equipe escolhida muda. */
  function invalidateGeneratedRoutes(){if(!state.generated)return;state.generated=null;document.querySelector('.distribution-results')?.remove();RoutePilotAgendaMap.render([]);const status=document.querySelector('.map-panel-heading span');if(status)status.textContent='Aguardando geração';}
  /** Aplica um filtro salvo sem reconstruir o formulário do atendimento. */
  function applyRouteTechnicianFilter(filter){const normalized=RoutePilotAgendaFilters.normalizeFilter(filter,activeTechnicians().map(item=>item.id));state.selected=new Set(normalized.technicianIds);state.activeRouteFilterId=normalized.id||null;invalidateGeneratedRoutes();refreshRouteTechnicianSelection();showToast(`Filtro aplicado: ${normalized.name}`);}
  /** Persiste todos os filtros de equipe e mantém somente um como padrão. */
  async function persistRouteTechnicianFilters(){await RoutePilotAgendaStorage.putMany('settings',state.routeFilters.map(filter=>({...filter,type:'routeTechnicianFilter'})));}
  ...
  function locationFeedback(candidate,{confirmed=false}={}){
    ...
    return `<span class="${confirmed?'location-confirmed':''}">${message}</span>${warning}${confirmed?'':`<button type="button" class="location-use-button" data-agenda-action="confirmLocation">Usar este endereço</button>`}${state.searchCanExpand&&!confirmed?'<button type="button" data-agenda-action="moreLocationOptions">Ver outras opções</button>':''}<button type="button" data-agenda-action="openManualAddressCorrection">Informar endereço e coordenadas</button><button type="button" data-agenda-action="selectLocationOnMap">${confirmed?'Ajustar no mapa':'Selecionar no mapa'}</button>${googleMapsCheckLink(candidate)}`;
  }
  ...
  function previewLocation(index,{confirm=false}={}){
    ...
    state.locationCandidate=candidate;state.confirmedLocation=null;if(confirm){confirmLocation();return;}RoutePilotAgendaMap.previewLocation(candidate);showLocationSearch(state.searchResults,locationFeedback(candidate));
  }
  ...
  /** Abre o cadastro manual sem apagar os demais campos que ja foram preenchidos na OS. */
  function openManualAddressCorrection(){
    document.querySelector('.address-correction-modal')?.remove();
    const root=$agenda('operationsContent'),candidate=state.confirmedLocation,address=$agenda('workOrderAddress')?.value.trim()||candidate?.formattedAddress||'',coords=candidate?.coords||[],selectedCity=candidate?.city||'',locality=candidate?.locality||'';
    const cityOptions=Object.keys(cityNames).map(id=>`<option value="${esc(id)}" ${id===selectedCity?'selected':''}>${esc(cityName(id))}</option>`).join('');
    const mapAdmin=globalThis.RoutePilotAuth?.hasCapability('canManageSharedMap'),destinationOptions=mapAdmin?'<option value="shared">Publicar para todos</option><option value="private">Salvar somente para mim</option>':'<option value="private">Salvar somente para mim</option><option value="request">Salvar e solicitar aprovação</option>';
    root?.insertAdjacentHTML('beforeend',`<div class="agenda-modal-backdrop address-correction-modal"><section class="agenda-modal address-correction-dialog" role="dialog" aria-modal="true" aria-label="Informar endereço e coordenadas"><div class="modal-heading"><div><small>AJUSTE GEOGRÁFICO</small><h2>Informar local e coordenadas</h2></div><button type="button" data-agenda-action="closeAddressCorrectionModal" aria-label="Fechar">×</button></div><p>O ponto privado fica visível só para você. Alterações compartilhadas passam pela autorização do administrador do mapa.</p><form id="manualAddressCorrectionForm" class="address-correction-form"><label>Tipo de item<select name="entityType"><option value="address">Endereço</option><option value="point">Ponto</option><option value="street">Rua</option><option value="neighborhood">Bairro/localidade</option></select></label><label>Destino<select name="saveMode">${destinationOptions}</select></label><label class="form-wide">Nome ou endereço completo<input name="formattedAddress" required maxlength="180" value="${esc(address)}" placeholder="Rua Exemplo, 180"></label><label>Latitude<input name="latitude" required inputmode="decimal" value="${coords[0]??''}" placeholder="-31.7392109"></label><label>Longitude<input name="longitude" required inputmode="decimal" value="${coords[1]??''}" placeholder="-52.3985395"></label><label>Cidade<select name="city" required><option value="">Selecione</option>${cityOptions}</select></label><label>Bairro ou localidade<input name="locality" maxlength="100" value="${esc(locality)}" placeholder="Fragata"></label><label>Fonte de conferência<input name="sourceName" maxlength="120" placeholder="Ex.: Prefeitura ou vistoria"></label><label>Link da fonte<input name="sourceUrl" type="url" maxlength="500" placeholder="https://..."></label><label class="form-wide">Motivo da alteração<textarea name="reason" maxlength="500" rows="2" placeholder="Explique o que precisa ser corrigido"></textarea></label><div class="address-correction-note form-wide">Coordenadas são WGS84. Nenhum dado de cliente, técnico ou OS entra na alteração geográfica.</div><div class="modal-actions form-wide"><button type="button" data-agenda-action="closeAddressCorrectionModal">Cancelar</button><button class="agenda-primary" type="submit">Salvar ajuste e usar</button></div></form></section></div>`);
    $agenda('manualAddressCorrectionForm')?.elements.namedItem('formattedAddress')?.focus();
  }

  /** Exibe todos os pontos pendentes que podem ser enviados para consolidacao da base. */
  function openAddressCorrectionsManager(){
    document.querySelector('.address-correction-modal')?.remove();
    const root=$agenda('operationsContent'),privateItems=RoutePilotAddressCorrectionsStorage.privateRecords(),requests=RoutePilotAddressCorrectionsStorage.requests(),approved=RoutePilotAddressCorrectionsStorage.approvedRecords(),mapAdmin=globalThis.RoutePilotAuth?.hasCapability('canReviewMapRequests');
    const itemRow=(item,badge,actions='')=>`<article><div><strong>${esc(item.formattedAddress)}</strong><span>${esc([item.locality,cityName(item.city)].filter(Boolean).join(' · '))}</span></div><code>${item.coords[0].toFixed(7)}, ${item.coords[1].toFixed(7)}</code><b>${esc(badge)}</b>${actions}</article>`;
    const privateRows=privateItems.map(item=>itemRow(item,'Só você')).join(''),requestRows=requests.map(item=>itemRow(item,item.status==='approved'?'Aprovado':item.status==='rejected'?'Rejeitado':'Aguardando',mapAdmin&&item.status==='pending'?`<span class="map-review-actions"><button data-agenda-action="approveMapRequest" data-id="${esc(item.id)}">Aprovar</button><button data-agenda-action="rejectMapRequest" data-id="${esc(item.id)}">Rejeitar</button></span>`:'' )).join(''),approvedRows=approved.map(item=>itemRow(item,'Compartilhado')).join('');
    root?.insertAdjacentHTML('beforeend',`<div class="agenda-modal-backdrop address-correction-modal"><section class="agenda-modal address-corrections-manager" role="dialog" aria-modal="true" aria-label="Ajustes geográficos"><div class="modal-heading"><div><small>REVISÃO GEOGRÁFICA</small><h2>Ajustes e solicitações</h2></div><button type="button" data-agenda-action="closeAddressCorrectionModal" aria-label="Fechar">×</button></div><p>${mapAdmin?'Você pode aprovar solicitações. Só pontos aprovados entram no mapa compartilhado.':'Seus ajustes privados não alteram o mapa dos demais usuários.'}</p><h3>PRIVADOS (${privateItems.length})</h3><div class="address-correction-list">${privateRows||'<p class="agenda-empty">Nenhum ajuste privado.</p>'}</div><h3>SOLICITAÇÕES (${requests.length})</h3><div class="address-correction-list">${requestRows||'<p class="agenda-empty">Nenhuma solicitação.</p>'}</div><h3>APROVADOS (${approved.length})</h3><div class="address-correction-list">${approvedRows||'<p class="agenda-empty">Nenhum ponto compartilhado.</p>'}</div><div class="modal-actions"><button type="button" data-agenda-action="closeAddressCorrectionModal">Fechar</button><button class="agenda-primary" type="button" data-agenda-action="exportAddressCorrections" ${privateItems.length?'':'disabled'}>Exportar meus ajustes</button></div></section></div>`);
  }

  /** Persiste o novo ponto e o utiliza imediatamente no atendimento atual. */
  async function saveManualAddressCorrection(form){
    const data=new FormData(form),latitude=Number(String(data.get('latitude')).replace(',','.')),longitude=Number(String(data.get('longitude')).replace(',','.')),city=String(data.get('city')),region=regionAtCoordinates(latitude,longitude),input={formattedAddress:data.get('formattedAddress'),entityType:data.get('entityType'),latitude,longitude,city,locality:data.get('locality'),region:region?.city===city?region.id:null,sourceName:data.get('sourceName'),sourceUrl:data.get('sourceUrl'),reason:data.get('reason')},mode=String(data.get('saveMode'));
    let record,request=null;if(mode==='shared')record=await RoutePilotAddressCorrectionsStorage.publish(input);else{record=await RoutePilotAddressCorrectionsStorage.save(input);if(mode==='request')request=await RoutePilotAddressCorrectionsStorage.submitRequest({...input,id:record.id});}
    state.addressCorrections=RoutePilotAddressCorrectionsStorage.privateRecords();state.geocodingService?.clearCache();document.querySelector('.address-correction-modal')?.remove();
    const addressInput=$agenda('workOrderAddress');if(addressInput)addressInput.value=record.formattedAddress;
    state.searchResults=[];state.searchCanExpand=false;state.searchWarning='';state.locationCandidate={...record,latitude:record.coords[0],longitude:record.coords[1],locationConfirmed:true};confirmLocation();
    const counter=$agenda('addressCorrectionCount');if(counter)counter.textContent=String(state.addressCorrections.length);showToast(mode==='shared'?'Local publicado no mapa compartilhado':mode==='request'?(request?.syncPending?'Ajuste salvo; a solicitação será enviada quando a conexão voltar':'Ajuste salvo e enviado para aprovação'):'Ajuste salvo somente para você');
  }
  ...
  function exportAddressCorrections(){
    const payload=RoutePilotAddressCorrectionsStorage.exportPayload(),blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a');link.href=url;link.download=`routepilot-enderecos-pendentes-${new Date().toISOString().slice(0,10)}.json`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);showToast(`${payload.count} endereço(s) exportado(s)`);
  }
  ...
  function renderAgenda(){
    ...
    const {startHour,endHour,timeAxisWidth,minColumnWidth}=AGENDA_LAYOUT,hourHeight=agendaHourHeight(),totalHeight=(endHour-startHour)*hourHeight;
    ...
    const canReorganize=items.length>1;
    return `<section class="agenda-view">${dateToolbar('Agenda diária')}<div class="agenda-toolbar"><span>${esc(todayLabel(state.date))}</span>${drawerAvailable?`<button class="unassigned-toggle" data-agenda-action="toggleUnassignedDrawer" aria-pressed="${state.unassignedOpen}">${state.unassignedOpen?'Ocultar':'Mostrar'} não agendadas <b>${unassigned.length}</b></button>`:''}<button class="agenda-reorganize-button" data-agenda-action="requestRouteReorganization" ${canReorganize?'':'disabled'} title="Escolher técnicos, turnos e modo de recálculo">Recalcular rotas</button><div class="agenda-filter-anchor"><button data-agenda-action="toggleAgendaFilter" aria-expanded="${state.filterOpen}">Técnicos: ${technicians.length} selecionados ▾</button>${state.filterOpen?renderAgendaFilter(allTechnicians):''}</div><button data-agenda-action="manageTechnicians">Gerenciar técnicos</button><button data-main-tab="create">Criar rota</button></div><div class="agenda-workspace-layout ${drawerVisible?'has-unassigned-drawer':''}">${drawer}<div class="agenda-board" style="grid-template-columns:${timeAxisWidth}px repeat(${columns.length},minmax(${minColumnWidth}px,1fr))"><div class="agenda-time-header"></div>${columns.map(item=>`<div class="agenda-tech-header" title="${esc(item.name)}"><strong>${esc(item.name)}</strong><small>${esc(item.serviceArea)}</small></div>`).join('')}<div class="agenda-time-axis" style="height:${totalHeight}px">${Array.from({length:endHour-startHour+1},(_,index)=>`<span style="top:${index*hourHeight}px">${String(startHour+index).padStart(2,'0')}:00</span>`).join('')}</div>${columns.map(technician=>`<div class="agenda-column" data-agenda-calendar-target="${esc(technician.id)}" style="height:${totalHeight}px;background-size:100% ${hourHeight}px">${items.filter(item=>item.technicianId===technician.id).map(item=>renderAgendaBlock(item,startHour,hourHeight)).join('')}</div>`).join('')}</div></div></section>`;
  }
  /** Ajusta as horas ao espaço vertical disponível entre notebooks e monitores grandes. */
  function agendaHourHeight(){const {startHour,endHour,minHourHeight,maxHourHeight,reservedViewportHeight}=AGENDA_LAYOUT,availableHeight=window.innerHeight-reservedViewportHeight;return Math.max(minHourHeight,Math.min(maxHourHeight,Math.floor(availableHeight/(endHour-startHour))));}
  /** Monta um cartão arrastável para uma OS que ainda não está na grade. */
  function renderUnassignedCard(order,reason){return `<article class="unassigned-card" draggable="${!order.locked}" data-agenda-unscheduled-drag="${esc(order.id)}"><div><strong>${esc(visitLabel(order))}</strong><small>${esc(serviceLabel(order.serviceType))} · ${esc(shiftLabel(order.shift))}${timeUnitsLabel(order)}</small><span>${esc(order.locality||order.address)}</span>${reason?`<em>${esc(reason)}</em>`:''}</div><button data-agenda-action="orderDetails" data-id="${esc(order.id)}">Detalhes</button></article>`;}
  ...
  function renderAgendaBlock(item,startHour,hourHeight){const top=(item.start-startHour*60)/60*hourHeight,height=Math.max(20,(item.end-item.start)/60*hourHeight),densityClass=height<48?' is-compact':'';return `<button class="agenda-block${densityClass}" draggable="${!item.order.locked}" style="top:${top}px;height:${height}px;--service-color:${CONFIG.SERVICE_TYPES[item.order.serviceType].color}" data-agenda-action="orderDetails" data-agenda-calendar-drag="${esc(item.technicianId)}:${esc(item.shiftId)}" data-order-id="${esc(item.order.id)}" data-id="${esc(item.order.id)}" title="${esc(`${CORE.minutesToTime(item.start)}–${CORE.minutesToTime(item.end)} · ${visitLabel(item.order)}`)}"><time>${CORE.minutesToTime(item.start)}–${CORE.minutesToTime(item.end)}</time><strong>${esc(visitLabel(item.order))}</strong><small>${esc(serviceLabel(item.order.serviceType))}${timeUnitsLabel(item.order)}</small><span>${esc(item.order.locality||item.order.address)}</span></button>`;}
  ...
  function renderAgendaPreview(){
    ...
    const scope=preview?.reorganization?`<p class="preview-scope"><strong>Escopo:</strong> ${esc(preview.scopeSummary)}</p>`:'',allocation=preview?.reorganization?'Todas as OS selecionadas permaneceram alocadas.':`${preview?.changes?.newOrders||0} novas OS encaixadas · ${preview?.candidate?.unallocated?.length||0} não alocadas`;
    return `<div class="agenda-modal-backdrop"><section class="agenda-modal" role="dialog" aria-modal="true" aria-label="Prévia da agenda"><div class="modal-heading"><div><small>PRÉVIA</small><h2>${preview?.candidate?'Revise antes de aplicar':existing?'Agenda já possui OS':'Enviar para agenda'}</h2></div><button data-agenda-action="cancelAgendaPreview">×</button></div>${preview?.candidate?`${scope}<div class="preview-summary"><p><strong>${preview.changes.technicianChanges}</strong> trocaram de técnico</p><p><strong>${preview.changes.timeChanges}</strong> mudaram de horário</p><p>${allocation}</p><p>Distância atual: ${preview.changes.currentDistance.toLocaleString('pt-BR',{maximumFractionDigits:1})} km · Nova: ${preview.changes.newDistance.toLocaleString('pt-BR',{maximumFractionDigits:1})} km</p></div>${changeRows?`<ul class="preview-changes">${changeRows}</ul>`:''}<div class="modal-actions"><button class="agenda-primary" data-agenda-action="applyAgenda">Aplicar nova agenda</button><button data-agenda-action="cancelAgendaPreview">Cancelar</button></div>`:`<div class="preview-summary"><p><strong>${existing}</strong> OS já existentes</p><p><strong>${newCount}</strong> OS da nova distribuição</p><p>Escolha como aplicar. A opção selecionada será calculada e salva na Agenda.</p></div>${existing?`<div class="agenda-choice"><button data-agenda-action="chooseAgendaMode" data-mode="reoptimize"><strong>Reotimizar agenda atual + novas OS</strong><span>Pode alterar técnico, ordem e horário de OS não bloqueadas.</span></button><button data-agenda-action="chooseAgendaMode" data-mode="fit"><strong>Manter agenda e encaixar novas</strong><span>Preserva a programação atual.</span></button></div>`:'<div class="modal-actions"><button class="agenda-primary" data-agenda-action="chooseAgendaMode" data-mode="replace">Aplicar na agenda</button><button data-agenda-action="cancelAgendaPreview">Cancelar</button></div>'}`}</section></div>`;
  }
  ...
  /** Permite escolher tecnicos, turnos e o tipo de recalculo antes da previa. */
  function renderRouteReorganizationConfirmation(){
    const selection=state.pendingRouteReorganization,schedules=state.agenda?.schedules||[],selectedTechnicians=new Set(selection.technicianIds),selectedShifts=new Set(selection.shiftIds);
    const technicians=[...new Map(schedules.filter(schedule=>schedule.items.length).map(schedule=>[schedule.technician.id,schedule.technician])).values()].sort((a,b)=>a.displayOrder-b.displayOrder);
    const targetSchedules=schedules.filter(schedule=>selectedTechnicians.has(schedule.technician.id)&&selectedShifts.has(schedule.shiftId)),visits=targetSchedules.reduce((sum,schedule)=>sum+schedule.items.length,0);
    const canApply=selection.mode==='redistribute'?selectedTechnicians.size>0&&selectedShifts.size>0&&visits>1:targetSchedules.some(schedule=>schedule.items.length>1);
    const technicianOptions=technicians.map(technician=>`<label><input type="checkbox" data-route-reorganization-technician="${esc(technician.id)}" ${selectedTechnicians.has(technician.id)?'checked':''}><span>${esc(technician.name)}<small>${esc(technician.serviceArea||'Sem base definida')}</small></span></label>`).join('');
    return `<div class="agenda-modal-backdrop"><section class="agenda-modal route-reorganization-confirmation" role="dialog" aria-modal="true" aria-label="Configurar recálculo das rotas"><div class="modal-heading"><div><small>AÇÃO MANUAL</small><h2>Recalcular rotas da Agenda</h2></div><button data-agenda-action="cancelRouteReorganization" aria-label="Fechar">×</button></div><fieldset class="route-reorganization-modes"><legend>Como recalcular</legend><label><input type="radio" name="routeReorganizationMode" value="reorder" ${selection.mode==='reorder'?'checked':''}><span><strong>Ordenar cada rota</strong><small>Mantém cada OS com o técnico e turno atuais.</small></span></label><label><input type="radio" name="routeReorganizationMode" value="redistribute" ${selection.mode==='redistribute'?'checked':''}><span><strong>Redistribuir entre selecionados</strong><small>Pode trocar OS não bloqueadas entre os técnicos e turnos escolhidos.</small></span></label></fieldset><div class="route-reorganization-heading"><strong>Técnicos</strong><span><button data-agenda-action="selectAllReorganizationTechnicians">Todos</button><button data-agenda-action="selectNoReorganizationTechnicians">Nenhum</button></span></div><div class="route-reorganization-technicians">${technicianOptions}</div><fieldset class="route-reorganization-shifts"><legend>Turnos incluídos</legend>${Object.entries(CONFIG.SHIFTS).filter(([id])=>id!=='any').map(([id,shift])=>`<label><input type="checkbox" data-route-reorganization-shift="${esc(id)}" ${selectedShifts.has(id)?'checked':''}> ${esc(shift.label)}</label>`).join('')}</fieldset><p>Escopo atual: <strong>${visits} visitas</strong> em <strong>${targetSchedules.length} rotas</strong>.</p><div class="route-recalculation-notice"><strong>Restrições operacionais continuam obrigatórias</strong><span>Capacidade, duração, técnico obrigatório, horários fixos, janelas, bloqueios e posições fixas serão respeitados. Você verá uma prévia antes de salvar.</span></div><div class="modal-actions"><button data-agenda-action="cancelRouteReorganization">Cancelar</button><button class="agenda-primary" data-agenda-action="confirmRouteReorganization" ${canApply?'':'disabled'}>Calcular prévia</button></div></section></div>`;
  }
  ...
  async function prepareAgendaCandidate(mode,{renderPreview=true}={}){
    ...
    const existingIds=new Set(existingItems.map(item=>item.order.id)),currentTechnicianIds=current?.schedules?.map(schedule=>schedule.technician.id)||[],selectedTechnicianIds=[...new Set([...state.selected,...currentTechnicianIds])],newOrders=sameDayOrders().filter(order=>!existingIds.has(order.id)),matrix=current?await buildScheduleMatrix(current.schedules,newOrders):state.generated.matrix;let candidate;
    if(mode==='replace'||!current)candidate=state.generated;
    else if(mode==='fit')candidate=CORE.allocateWorkOrders(newOrders,state.technicians,{matrix,selectedTechnicianIds,initialSchedules:current.schedules});
    else{
      ...
      candidate=optimizeDistributedSchedules(CORE.allocateWorkOrders(unlocked,state.technicians,{matrix,selectedTechnicianIds,initialSchedules:lockedSchedules}),matrix);
    }
    ...
    state.pendingAgenda={mode,candidate,changes:{technicianChanges,timeChanges,newOrders:afterItems.filter(item=>!before.has(item.order.id)).length,currentDistance:current?.schedules?.reduce((sum,schedule)=>sum+(Number(schedule.distanceKm)||0),0)||0,newDistance:candidate.schedules.reduce((sum,schedule)=>sum+(Number(schedule.distanceKm)||0),0),details}};if(renderPreview)render();return state.pendingAgenda;
  }
  /** Aplica a estratégia escolhida no primeiro clique e informa qualquer falha. */
  async function chooseAndApplyAgendaMode(mode,button){button.disabled=true;button.setAttribute('aria-busy','true');showToast('Atualizando agenda...');try{await prepareAgendaCandidate(mode,{renderPreview:false});await applyAgenda();}catch(error){console.error('Falha ao atualizar a Agenda',error);state.pendingAgenda={mode:null};render();showToast('Não foi possível atualizar a Agenda. Tente novamente.');}}
  /** Salva a distribuição somente depois da confirmação da prévia. */
  async function applyAgenda(){const preview=state.pendingAgenda,candidate=preview?.candidate;if(!candidate)return;state.agenda={date:state.date,schedules:candidate.schedules,unallocated:candidate.unallocated,updatedAt:new Date().toISOString()};await RoutePilotAgendaStorage.saveAgenda(state.agenda);state.pendingAgenda=null;state.generated=null;state.tab='agenda';render();$agenda('operationsWorkspace').scrollTop=0;showToast(preview.reorganization?'Rotas selecionadas recalculadas':'Agenda atualizada');}
  ...
  /** Resume as diferenças entre a Agenda atual e uma prévia ainda não persistida. */
  function summarizeAgendaCandidate(current,candidate,meta={}){
    const beforeItems=current?.schedules?.flatMap(schedule=>schedule.items.map(item=>({...item,technicianId:schedule.technician.id,shiftId:schedule.shiftId})))||[],before=new Map(beforeItems.map(item=>[item.order.id,item]));
    const afterItems=candidate.schedules.flatMap(schedule=>schedule.items.map(item=>({...item,technicianId:schedule.technician.id,shiftId:schedule.shiftId})));let technicianChanges=0,timeChanges=0;const details=[];
    afterItems.forEach(item=>{const old=before.get(item.order.id);if(!old)return;const changes=[];if(old.technicianId!==item.technicianId){technicianChanges++;changes.push(`${technicianById(old.technicianId)?.name||'Sem colaborador'} → ${technicianById(item.technicianId)?.name||'Sem colaborador'}`);}if(old.shiftId!==item.shiftId)changes.push(`${shiftLabel(old.shiftId)} → ${shiftLabel(item.shiftId)}`);if(old.start!==item.start){timeChanges++;changes.push(`${CORE.minutesToTime(old.start)} → ${CORE.minutesToTime(item.start)}`);}if(changes.length)details.push({label:visitLabel(item.order),text:changes.join(' · ')});});
    return {mode:meta.mode||'recalculate',candidate,reorganization:Boolean(meta.reorganization),scopeSummary:meta.scopeSummary||'',changes:{technicianChanges,timeChanges,newOrders:afterItems.filter(item=>!before.has(item.order.id)).length,currentDistance:current?.schedules?.reduce((sum,schedule)=>sum+(Number(schedule.distanceKm)||0),0)||0,newDistance:candidate.schedules.reduce((sum,schedule)=>sum+(Number(schedule.distanceKm)||0),0),details}};
  }
  /** Calcula somente o escopo escolhido e abre uma prévia sem salvar a Agenda. */
  async function reorganizeAgendaRoutes(){
    const selection=state.pendingRouteReorganization,current=state.agenda,schedules=current?.schedules||[];if(!selection)return;
    const technicianIds=[...new Set(selection.technicianIds)],shiftIds=[...new Set(selection.shiftIds)],selectedTechnicians=new Set(technicianIds),selectedShifts=new Set(shiftIds);
    const inScope=schedule=>selectedTechnicians.has(schedule.technician.id)&&selectedShifts.has(schedule.shiftId),targetSchedules=schedules.filter(inScope),visits=targetSchedules.reduce((sum,schedule)=>sum+schedule.items.length,0);
    if(!technicianIds.length||!shiftIds.length||visits<2||selection.mode==='reorder'&&!targetSchedules.some(schedule=>schedule.items.length>1)){showToast('Selecione técnicos e turnos com uma rota que possa ser recalculada');return;}
    showToast('Calculando prévia das rotas selecionadas...');
    try{
      const matrix=await buildScheduleMatrix(targetSchedules);let candidate;
      if(selection.mode==='reorder'){
        const result=CORE.reorganizeTechnicianSchedules(schedules,{matrix,optimizeRoute:RoutePilotRouteOptimizer.optimizeRoute,selectedTechnicianIds:technicianIds,selectedShiftIds:shiftIds});
        candidate={schedules:result.schedules,unallocated:current.unallocated||[],total:visits,allocated:visits};
      }else{
        const protectedSchedules=targetSchedules.filter(schedule=>schedule.items.some(item=>item.order.locked||Number.isInteger(Number(item.order.fixedPosition))&&Number(item.order.fixedPosition)>0)),protectedKeys=new Set(protectedSchedules.map(schedule=>`${schedule.technician.id}:${schedule.shiftId}`));
        const redistributable=targetSchedules.filter(schedule=>!protectedKeys.has(`${schedule.technician.id}:${schedule.shiftId}`)),untouched=[...schedules.filter(schedule=>!inScope(schedule)),...protectedSchedules];
        const movable=redistributable.flatMap(schedule=>schedule.items.map(item=>item.order)),availableTechnicians=state.technicians.map(technician=>({...technician,defaultShifts:(technician.defaultShifts||[]).filter(shiftId=>!protectedKeys.has(`${technician.id}:${shiftId}`))}));
        if(movable.length<2)throw new Error('O escopo livre possui menos de duas OS; rotas bloqueadas permanecem intactas.');
        const distributed=CORE.allocateWorkOrders(movable,availableTechnicians,{matrix,selectedTechnicianIds:technicianIds,selectedShiftIds:shiftIds});
        if(distributed.unallocated.length)throw new Error(`Não foi possível encaixar ${distributed.unallocated.length} OS sem violar as restrições.`);
        const optimized=optimizeDistributedSchedules(distributed,matrix),merged=[...untouched,...optimized.schedules].sort((a,b)=>a.technician.displayOrder-b.technician.displayOrder||Object.keys(CONFIG.SHIFTS).indexOf(a.shiftId)-Object.keys(CONFIG.SHIFTS).indexOf(b.shiftId));
        candidate={...optimized,schedules:merged,unallocated:current.unallocated||[]};
      }
      const technicianCount=targetSchedules.reduce((ids,schedule)=>ids.add(schedule.technician.id),new Set()).size,scopeSummary=`${technicianCount} técnico${technicianCount===1?'':'s'} · ${shiftIds.map(shiftLabel).join(' e ')}`;
      state.pendingRouteReorganization=null;state.pendingAgenda=summarizeAgendaCandidate(current,candidate,{mode:selection.mode,reorganization:true,scopeSummary});render({preserveAgendaScroll:true});
    }catch(error){console.error('Falha ao recalcular rotas',error);state.pendingRouteReorganization=null;render({preserveAgendaScroll:true});showToast(error.message||'Não foi possível recalcular as rotas agora');}
  }
  ...
  async function handleClick(event){
    ...
    if(action==='selectAllRouteTechnicians'){state.selected=new Set(activeTechnicians().map(item=>item.id));state.activeRouteFilterId=null;invalidateGeneratedRoutes();document.querySelectorAll('[data-agenda-technician]').forEach(input=>input.checked=true);updateRouteTechnicianSelection();}
    if(action==='selectNoRouteTechnicians'){state.selected.clear();state.activeRouteFilterId=null;invalidateGeneratedRoutes();document.querySelectorAll('[data-agenda-technician]').forEach(input=>input.checked=false);updateRouteTechnicianSelection();}
    if(action==='applyRouteFilter'){const filter=state.routeFilters.find(item=>item.id===button.dataset.id);if(filter)applyRouteTechnicianFilter(filter);}
    if(action==='updateRouteFilter'){const filter=state.routeFilters.find(item=>item.id===button.dataset.id);if(filter){state.routeFilters=RoutePilotAgendaFilters.saveFilter(state.routeFilters,{...filter,technicianIds:[...state.selected]});await persistRouteTechnicianFilters();state.activeRouteFilterId=filter.id;refreshRouteTechnicianSelection();showToast('Filtro atualizado com a seleção atual');}}
    if(action==='defaultRouteFilter'){state.routeFilters=RoutePilotAgendaFilters.setDefault(state.routeFilters,button.dataset.id);await persistRouteTechnicianFilters();refreshRouteTechnicianSelection();showToast('Filtro padrão atualizado');}
    if(action==='deleteRouteFilter'){await RoutePilotAgendaStorage.removeRouteTechnicianFilter(button.dataset.id);state.routeFilters=RoutePilotAgendaFilters.removeFilter(state.routeFilters,button.dataset.id);if(state.activeRouteFilterId===button.dataset.id)state.activeRouteFilterId=null;refreshRouteTechnicianSelection();showToast('Filtro excluído');}
    ...
    if(action==='chooseAgendaMode')await chooseAndApplyAgendaMode(button.dataset.mode,button);
    ...
    if(action==='requestRouteReorganization'){const schedules=(state.agenda?.schedules||[]).filter(schedule=>schedule.items.length);state.pendingRouteReorganization={mode:'reorder',technicianIds:[...new Set(schedules.map(schedule=>schedule.technician.id))],shiftIds:[...new Set(schedules.map(schedule=>schedule.shiftId))]};render({preserveAgendaScroll:true});}
    if(action==='cancelRouteReorganization'){state.pendingRouteReorganization=null;render({preserveAgendaScroll:true});}
    if(action==='selectAllReorganizationTechnicians'){state.pendingRouteReorganization.technicianIds=[...new Set((state.agenda?.schedules||[]).filter(schedule=>schedule.items.length).map(schedule=>schedule.technician.id))];render({preserveAgendaScroll:true});}
    if(action==='selectNoReorganizationTechnicians'){state.pendingRouteReorganization.technicianIds=[];render({preserveAgendaScroll:true});}
    if(action==='confirmRouteReorganization')await reorganizeAgendaRoutes();
    if(action==='previewLocation')previewLocation(Number(button.dataset.index),{confirm:true});
    ...
    if(action==='openManualAddressCorrection')openManualAddressCorrection();
    if(action==='manageAddressCorrections')openAddressCorrectionsManager();
    if(action==='closeAddressCorrectionModal')document.querySelector('.address-correction-modal')?.remove();
  ...
  async function handleSubmit(event){
    ...
    if(event.target.id==='manualAddressCorrectionForm'){event.preventDefault();try{await saveManualAddressCorrection(event.target);}catch(error){showToast(error.message||'Não foi possível salvar o ajuste local');}}
    ...
    if(event.target.id==='routeFilterForm'){event.preventDefault();const data=new FormData(event.target),filter={id:makeId('route_filter'),name:String(data.get('name')).trim(),technicianIds:[...state.selected],isDefault:data.has('isDefault')};state.routeFilters=RoutePilotAgendaFilters.saveFilter(state.routeFilters,filter);await persistRouteTechnicianFilters();state.activeRouteFilterId=filter.id;refreshRouteTechnicianSelection();showToast('Filtro da equipe salvo');}
  ...
  async function handleChange(event){
    ...
    if(event.target.dataset.agendaTechnician){event.target.checked?state.selected.add(event.target.dataset.agendaTechnician):state.selected.delete(event.target.dataset.agendaTechnician);state.activeRouteFilterId=null;invalidateGeneratedRoutes();updateRouteTechnicianSelection();}
    if(event.target.id==='routeSavedFilter'){const filter=state.routeFilters.find(item=>item.id===event.target.value);if(filter)applyRouteTechnicianFilter(filter);else state.activeRouteFilterId=null;}
    ...
    if(event.target.name==='routeReorganizationMode'&&state.pendingRouteReorganization){state.pendingRouteReorganization.mode=event.target.value==='redistribute'?'redistribute':'reorder';render({preserveAgendaScroll:true});}
    if(event.target.dataset.routeReorganizationTechnician&&state.pendingRouteReorganization){const ids=new Set(state.pendingRouteReorganization.technicianIds);event.target.checked?ids.add(event.target.dataset.routeReorganizationTechnician):ids.delete(event.target.dataset.routeReorganizationTechnician);state.pendingRouteReorganization.technicianIds=[...ids];render({preserveAgendaScroll:true});}
    if(event.target.dataset.routeReorganizationShift&&state.pendingRouteReorganization){const ids=new Set(state.pendingRouteReorganization.shiftIds);event.target.checked?ids.add(event.target.dataset.routeReorganizationShift):ids.delete(event.target.dataset.routeReorganizationShift);state.pendingRouteReorganization.shiftIds=[...ids];render({preserveAgendaScroll:true});}
  ...
  function handleDragOver(event){
    ...
    if(state.drag.scope==='agenda'){const {startHour,endHour}=AGENDA_LAYOUT,hourHeight=agendaHourHeight(),rect=target.getBoundingClientRect(),minutes=Math.max(startHour*60,Math.min(endHour*60,Math.round((startHour*60+(event.clientY-rect.top)/hourHeight*60)/15)*15));target.dataset.dropTime=CORE.minutesToTime(minutes);target.style.setProperty('--agenda-drop-y',`${(minutes-startHour*60)/60*hourHeight}px`);}
  }
  ...
  function handleDrop(event){
    ...
    const column=event.target.closest('[data-agenda-calendar-target]');if(!column)return;event.preventDefault();const {startHour,endHour}=AGENDA_LAYOUT,hourHeight=agendaHourHeight(),rect=column.getBoundingClientRect(),targetStart=Math.max(startHour*60,Math.min(endHour*60,Math.round((startHour*60+(event.clientY-rect.top)/hourHeight*60)/15)*15)),targetTechnician=technicianById(column.dataset.agendaCalendarTarget),preferredShift=shiftAtMinute(targetTechnician,targetStart);if(!preferredShift){handleDragEnd();showToast('Escolha um horário entre 08:00–12:00 ou 13:00–18:00');return;}queueScheduleMove({scope:'agenda',orderId:drag.orderId,sourceKey:drag.scheduleKey,targetTechnicianId:targetTechnician.id,targetShiftId:preferredShift,targetIndex:Number.MAX_SAFE_INTEGER,targetStart});
  }
