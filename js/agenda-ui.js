...
const RoutePilotAgenda=(()=>{
  ...
  const state={tab:'map',date:new Date().toISOString().slice(0,10),technicians:[],orders:[],selected:new Set(),locationKey:null,generated:null,agenda:null,manager:false,detailId:null,pendingAgenda:null,drag:null};
  ...
  async function init(){
    ...
    state.technicians=await RoutePilotAgendaStorage.ensureDefaultTechnicians();state.orders=await RoutePilotAgendaStorage.all('workOrders');state.selected=new Set(activeTechnicians().map(item=>item.id));state.agenda=await RoutePilotAgendaStorage.getAgenda(state.date)||null;
    bind();renderTabs();
  }
  /** Registra os eventos próprios uma única vez. */
  function bind(){
    document.addEventListener('click',handleClick);document.addEventListener('submit',handleSubmit);document.addEventListener('input',handleInput);document.addEventListener('change',handleChange);document.addEventListener('dragstart',handleDragStart);document.addEventListener('dragover',handleDragOver);document.addEventListener('drop',handleDrop);
  }
  ...
  function render(){
    renderTabs();const root=$agenda('operationsContent');if(!root)return;
  ...
  function renderAgenda(){
    const technicians=activeTechnicians(),agenda=state.agenda,items=agenda?.schedules?.flatMap(schedule=>schedule.items.map(item=>({...item,technicianId:schedule.technician.id})))||[];
    const startHour=6,endHour=19,rowHeight=64,totalHeight=(endHour-startHour)*rowHeight;
    const columns=[...technicians,{id:'unassigned',name:'Sem colaborador',serviceArea:''}];
    return `<section class="agenda-view">${dateToolbar('Agenda diária')}<div class="agenda-toolbar"><span>${esc(todayLabel(state.date))}</span><button data-agenda-action="manageTechnicians">Gerenciar técnicos</button><button data-main-tab="create">Criar rota</button></div><div class="agenda-board"><div class="agenda-time-header"></div>${columns.map(item=>`<div class="agenda-tech-header"><strong>${esc(item.name)}</strong><small>${esc(item.serviceArea||'OS não alocadas')}</small></div>`).join('')}<div class="agenda-time-axis" style="height:${totalHeight}px">${Array.from({length:endHour-startHour+1},(_,index)=>`<span style="top:${index*rowHeight}px">${String(startHour+index).padStart(2,'0')}:00</span>`).join('')}</div>${columns.map(technician=>`<div class="agenda-column" style="height:${totalHeight}px;background-size:100% ${rowHeight}px">${items.filter(item=>item.technicianId===technician.id).map(item=>renderAgendaBlock(item,startHour,rowHeight)).join('')}${technician.id==='unassigned'?(agenda?.unallocated||[]).map((entry,index)=>`<button class="agenda-unassigned" style="top:${index*58+8}px" data-agenda-action="orderDetails" data-id="${entry.order.id}"><strong>OS ${esc(entry.order.number)}</strong><small>${esc(entry.reason)}</small></button>`).join(''):''}</div>`).join('')}</div></section>`;
  }
  /** Posiciona um bloco da agenda de acordo com início e duração. */
  function renderAgendaBlock(item,startHour,rowHeight){const top=(item.start-startHour*60)/60*rowHeight,height=Math.max(34,(item.end-item.start)/60*rowHeight);return `<button class="agenda-block" style="top:${top}px;height:${height}px;--service-color:${CONFIG.SERVICE_TYPES[item.order.serviceType].color}" data-agenda-action="orderDetails" data-id="${item.order.id}"><time>${CORE.minutesToTime(item.start)}–${CORE.minutesToTime(item.end)}</time><strong>OS ${esc(item.order.number)}</strong><small>${esc(serviceLabel(item.order.serviceType))}</small><span>${esc(item.order.locality||item.order.address)}</span></button>`;}
  ...
  function renderAgendaPreview(){
    ...
    const changeRows=preview?.changes?.details?.slice(0,8).map(item=>`<li><strong>OS ${esc(item.number)}</strong><span>${esc(item.text)}</span></li>`).join('')||'';
  ...
  async function addWorkOrder(form){
    const data=new FormData(form),candidate={number:String(data.get('number')).trim()};if(CORE.findDuplicateWorkOrder(state.orders.filter(order=>!order.archived),candidate)){showToast('Esta OS já está cadastrada');return;}
    let place=state.locationKey?comparePlace(state.locationKey):null;
    if(!place){try{const matches=comparePlaceMatches(data.get('address'));place=matches.length===1?matches[0]:await resolveLocalRouteAddress(data.get('address'));if(!place.key)addTransientComparePlace(place);}catch(error){showToast(error.message||'Localização não encontrada');return;}}
    const fixedPosition=Number(data.get('fixedPosition'));
    const order={id:makeId('os'),number:candidate.number,date:state.date,serviceType:data.get('serviceType'),address:String(data.get('address')).trim(),coords:place.coords.map(Number),locality:place.name||String(data.get('locality')||''),city:place.city||'',shift:data.get('shift'),highPriority:data.has('highPriority'),locked:data.has('locked'),fixedPosition:Number.isInteger(fixedPosition)&&fixedPosition>0?fixedPosition:null,requiredTechnicianId:data.get('requiredTechnicianId')||null,note:String(data.get('note')||'').trim(),timeConstraint:{type:data.get('timeType'),start:data.get('timeStart')||null,end:data.get('timeEnd')||null},createdAt:new Date().toISOString()};
    state.orders.push(order);await RoutePilotAgendaStorage.put('workOrders',order);state.locationKey=null;state.generated=null;form.reset();render();showToast(`OS ${order.number} adicionada`);
  }
  ...
  function shareOrder(id){const order=state.orders.find(item=>item.id===id);if(!order)return;state.detailId=null;open('map');setTimeout(()=>openLocationShare(locationShareContext(order.coords[0],order.coords[1],{name:order.locality||`OS ${order.number}`,city:order.city})),0);}
  ...
  async function handleClick(event){
    ...
    if(action==='chooseLocation'){state.locationKey=button.dataset.key;const place=comparePlace(state.locationKey);$agenda('workOrderAddress').value=place.name;$agenda('workOrderSuggestions').hidden=true;}
  }
  ...
  function handleInput(event){if(event.target.id==='workOrderAddress'){state.locationKey=null;const matches=comparePlaceMatches(event.target.value).slice(0,8),panel=$agenda('workOrderSuggestions');panel.hidden=!event.target.value;panel.innerHTML=matches.map(item=>`<button type="button" data-agenda-action="chooseLocation" data-key="${esc(item.key)}"><strong>${esc(item.name)}</strong><small>${esc(cityName(item.city))} · ${esc(byRegion[item.region]?.name||'')}</small></button>`).join('');}}
