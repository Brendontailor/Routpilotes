...
const RoutePilotAgenda=(()=>{
  ...
  /** Confirma a acao manual que reorganiza todas as rotas do dia. */
  function renderRouteReorganizationConfirmation(){
    const eligible=(state.agenda?.schedules||[]).filter(schedule=>schedule.items.length>1),routes=eligible.length,visits=eligible.reduce((sum,schedule)=>sum+schedule.items.length,0);
    return `<div class="agenda-modal-backdrop"><section class="agenda-modal route-reorganization-confirmation" role="dialog" aria-modal="true" aria-label="Confirmar reorganização das rotas"><div class="modal-heading"><div><small>AÇÃO MANUAL</small><h2>Reorganizar todas as rotas?</h2></div><button data-agenda-action="cancelRouteReorganization">×</button></div><p>O RoutePilot recalculará <strong>${visits} visitas</strong> em <strong>${routes} rotas</strong>, procurando uma ordem melhor dentro da programação de cada técnico.</p><div class="route-recalculation-notice"><strong>Nenhuma OS trocará de técnico ou turno</strong><span>Horários fixos, janelas, bloqueios e posições fixas serão respeitados. Rotas que não puderem ser melhoradas permanecerão como estão.</span></div><div class="modal-actions"><button data-agenda-action="cancelRouteReorganization">Cancelar</button><button class="agenda-primary" data-agenda-action="confirmRouteReorganization">Reorganizar agora</button></div></section></div>`;
  }
  ...
  /** Recalcula manualmente a ordem de todas as rotas e salva a Agenda compartilhada. */
  async function reorganizeAgendaRoutes(){
    const schedules=state.agenda?.schedules||[];if(!schedules.some(schedule=>schedule.items.length>1)){state.pendingRouteReorganization=false;render({preserveAgendaScroll:true});showToast('Não há rotas com duas ou mais visitas para reorganizar');return;}
    showToast('Reorganizando as rotas de cada técnico...');
    try{
      const matrix=await buildScheduleMatrix(schedules),result=CORE.reorganizeTechnicianSchedules(schedules,{matrix,optimizeRoute:RoutePilotRouteOptimizer.optimizeRoute});
      state.agenda={...state.agenda,schedules:result.schedules,updatedAt:new Date().toISOString()};await RoutePilotAgendaStorage.saveAgenda(state.agenda);state.pendingRouteReorganization=false;render({preserveAgendaScroll:true});
      const message=result.changedSchedules?`${result.changedSchedules} rota${result.changedSchedules===1?'':'s'} reorganizada${result.changedSchedules===1?'':'s'}`:'As rotas já estavam na melhor ordem disponível';showToast(result.failedSchedules?`${message}; ${result.failedSchedules} mantida${result.failedSchedules===1?'':'s'} por restrições`:message);
    }catch(error){console.error('Falha ao reorganizar rotas',error);state.pendingRouteReorganization=false;render({preserveAgendaScroll:true});showToast('Não foi possível reorganizar as rotas agora');}
  }
  ...
  async function handleClick(event){
    ...
    if(action==='requestRouteReorganization'){state.pendingRouteReorganization=true;render({preserveAgendaScroll:true});}
    if(action==='cancelRouteReorganization'){state.pendingRouteReorganization=false;render({preserveAgendaScroll:true});}
