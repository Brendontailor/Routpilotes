...
const RoutePilotAgenda=(()=>{
  ...
  function openManualAddressCorrection(){
    ...
    root?.insertAdjacentHTML('beforeend',`<div class="agenda-modal-backdrop address-correction-modal"><section class="agenda-modal address-correction-dialog" role="dialog" aria-modal="true" aria-label="Informar endereço e coordenadas"><div class="modal-heading"><div><small>AJUSTE LOCAL</small><h2>Informar endereço e coordenadas</h2></div><button type="button" data-agenda-action="closeAddressCorrectionModal" aria-label="Fechar">×</button></div><p>Use este formulário quando a busca não conhecer um endereço. O ponto ficará disponível neste computador e entrará na lista pendente para exportação.</p><form id="manualAddressCorrectionForm" class="address-correction-form"><label class="form-wide">Endereço completo<input name="formattedAddress" required maxlength="180" value="${esc(address)}" placeholder="Rua Exemplo, 180"></label><label>Latitude<input name="latitude" required inputmode="decimal" value="${coords[0]??''}" placeholder="-31.7392109"></label><label>Longitude<input name="longitude" required inputmode="decimal" value="${coords[1]??''}" placeholder="-52.3985395"></label><label>Cidade<select name="city" required><option value="">Selecione</option>${cityOptions}</select></label><label>Bairro ou localidade<input name="locality" maxlength="100" value="${esc(locality)}" placeholder="Fragata"></label><div class="address-correction-note form-wide">A exportação guarda coordenadas WGS84, GeoJSON e link do mapa. Cliente, técnico e observações da OS não são incluídos.</div><div class="modal-actions form-wide"><button type="button" data-agenda-action="closeAddressCorrectionModal">Cancelar</button><button class="agenda-primary" type="submit">Salvar ajuste e usar</button></div></form></section></div>`);
  ...
  function openAddressCorrectionsManager(){
    document.querySelector('.address-correction-modal')?.remove();
    const root=$agenda('operationsContent'),items=state.addressCorrections.map(item=>`<article><div><strong>${esc(item.formattedAddress)}</strong><span>${esc([item.locality,cityName(item.city)].filter(Boolean).join(' · '))}</span></div><code>${item.coords[0].toFixed(7)}, ${item.coords[1].toFixed(7)}</code><b>Pendente</b></article>`).join('');
    root?.insertAdjacentHTML('beforeend',`<div class="agenda-modal-backdrop address-correction-modal"><section class="agenda-modal address-corrections-manager" role="dialog" aria-modal="true" aria-label="Ajustes locais de endereços"><div class="modal-heading"><div><small>REVISÃO GEOGRÁFICA</small><h2>Ajustes locais <span>${state.addressCorrections.length}</span></h2></div><button type="button" data-agenda-action="closeAddressCorrectionModal" aria-label="Fechar">×</button></div><p>Estes endereços ainda não fazem parte da base oficial do RoutePilot. Exporte o arquivo completo para revisão e integração posterior.</p><div class="address-correction-list">${items||'<p class="agenda-empty">Nenhum endereço pendente neste computador.</p>'}</div><div class="modal-actions"><button type="button" data-agenda-action="closeAddressCorrectionModal">Fechar</button><button class="agenda-primary" type="button" data-agenda-action="exportAddressCorrections" ${state.addressCorrections.length?'':'disabled'}>Exportar lista completa</button></div></section></div>`);
  }

  /** Persiste o novo ponto e o utiliza imediatamente no atendimento atual. */
  async function saveManualAddressCorrection(form){
    const data=new FormData(form),latitude=Number(String(data.get('latitude')).replace(',','.')),longitude=Number(String(data.get('longitude')).replace(',','.')),city=String(data.get('city')),region=regionAtCoordinates(latitude,longitude),record=await RoutePilotAddressCorrectionsStorage.save({formattedAddress:data.get('formattedAddress'),latitude,longitude,city,locality:data.get('locality'),region:region?.city===city?region.id:null});
    state.addressCorrections=RoutePilotAddressCorrectionsStorage.cached();state.geocodingService?.clearCache();document.querySelector('.address-correction-modal')?.remove();
    ...
    const counter=$agenda('addressCorrectionCount');if(counter)counter.textContent=String(state.addressCorrections.length);showToast('Endereço salvo nos ajustes locais e confirmado');
  }
