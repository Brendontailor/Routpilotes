...
const RoutePilotAgenda=(()=>{
  ...
  function renderMoveConfirmation(){
    const move=state.pendingMove,source=move.sourceKey?technicianById(move.sourceKey.split(':')[0]):null,target=technicianById(move.targetTechnicianId),reminder=CORE.assignmentReminder(target,move.order);
    return `<div class="agenda-modal-backdrop"><section class="agenda-modal move-confirmation" role="dialog" aria-modal="true" aria-label="Confirmar mudança de técnico"><div class="modal-heading"><div><small>CONFIRMAÇÃO</small><h2>Mudar técnico?</h2></div><button data-agenda-action="cancelScheduleMove">×</button></div><p>Mover <strong>${esc(visitLabel(move.order))}</strong> de <b>${esc(source?.name||'OS não agendadas')}</b> para <b>${esc(target?.name||'Técnico')}</b>, no turno da ${esc(shiftLabel(move.targetShiftId).toLowerCase())}?</p><div class="route-recalculation-notice"><strong>As rotas afetadas serão recalculadas</strong><span>O RoutePilot validará capacidade, horários, distância e restrições antes de salvar.</span></div>${reminder?`<p class="area-reminder">${esc(reminder)}</p>`:''}<div class="modal-actions"><button data-agenda-action="cancelScheduleMove">Cancelar</button><button class="agenda-primary" data-agenda-action="confirmScheduleMove">Mover e recalcular</button></div></section></div>`;
  }
  ...
  function queueScheduleMove({scope,orderId,sourceKey=null,targetTechnicianId,targetShiftId,targetIndex}){
    ...
    const target=technicianById(targetTechnicianId);if(!target){showToast('Técnico de destino não encontrado');return;}const shift=compatibleShift(order,target,targetShiftId);if(!shift){showToast(moveFailureMessage('SHIFT_CONFLICT'));return;}
    if(sourceKey===`${target.id}:${shift}`)return;
    state.drag=null;state.pendingMove={scope,order,sourceKey,targetTechnicianId:target.id,targetShiftId:shift,targetIndex};render({preserveAgendaScroll:scope==='agenda'});
  }
  ...
  async function confirmScheduleMove(){
    ...
    else{state.agenda={date:state.date,schedules:updated,unallocated:(state.agenda?.unallocated||[]).filter(item=>item.order.id!==move.order.id),updatedAt:new Date().toISOString()};await RoutePilotAgendaStorage.saveAgenda(state.agenda);}
  ...
  function handleDragOver(event){
    ...
    event.preventDefault();if(event.dataTransfer)event.dataTransfer.dropEffect='move';document.querySelectorAll('.is-drop-target').forEach(item=>item.classList.remove('is-drop-target'));target.classList.add('is-drop-target');
  }
  ...
  function handleDrop(event){
    ...
    const column=event.target.closest('[data-agenda-calendar-target]');if(!column)return;event.preventDefault();const rect=column.getBoundingClientRect(),dropHour=6+((event.clientY-rect.top)/64),preferredShift=dropHour>=12.5?'afternoon':'morning';queueScheduleMove({scope:'agenda',orderId:drag.orderId,sourceKey:drag.scheduleKey,targetTechnicianId:column.dataset.agendaCalendarTarget,targetShiftId:preferredShift,targetIndex:Number.MAX_SAFE_INTEGER});
  }
  /** Limpa realces visuais quando o usuário cancela ou encerra o arrasto. */
  function handleDragEnd(){document.querySelectorAll('.is-dragging,.is-drop-target').forEach(item=>item.classList.remove('is-dragging','is-drop-target'));state.drag=null;}
  /** Esc fecha primeiro a confirmação operacional que estiver aberta. */
  function handleKeydown(event){if(event.key!=='Escape')return;if(state.pendingMove){state.pendingMove=null;render({preserveAgendaScroll:state.tab==='agenda'});}else if(state.pendingSuggestion){state.pendingSuggestion=null;render({preserveAgendaScroll:state.tab==='agenda'});}}
