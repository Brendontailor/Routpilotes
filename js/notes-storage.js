/* Persistencia privada e offline das anotacoes operacionais. */
const RoutePilotNotes=(()=>{
  ...
  /** Localiza uma anotacao e confirma que ela pertence a conta atual. */
  async function byId(id){const note=await transaction('readonly',store=>store.get(id));return note?.userId===currentUserId()?note:null;}
  ...
  /** Entrega somente anotacoes da conta atual e associa registros locais legados. */
  async function localForCurrentUser(){const userId=requireUser(),records=await localAll(),visible=[];for(const record of records){if(record.userId&&record.userId!==userId)continue;const note=record.userId?record:{...record,userId,syncStatus:'pending'};if(!record.userId)await transaction('readwrite',store=>store.put(note));visible.push(note);}return visible;}
  ...
  async function syncCloud(){const userId=requireUser(),cloud=globalThis.RoutePilotCloudSync;if(!cloud)return false;const local=await localForCurrentUser();for(const note of local.filter(item=>item.syncStatus!=='synced')){try{await cloud.upsert('notes',note);await markSynced(note);}catch(error){await transaction('readwrite',store=>store.put({...note,syncStatus:'failed',syncError:String(error?.message||'sync_failed').slice(0,80)}));throw error;}}const remote=await cloud.list('notes'),localMap=new Map((await localForCurrentUser()).map(note=>[note.id,note]));for(const record of remote){if(record._deleted){const current=localMap.get(record.id);if(current?.syncStatus==='synced')await transaction('readwrite',store=>store.delete(record.id));continue;}const note={...record,userId,syncStatus:'synced',syncError:null},current=localMap.get(note.id);if(!current||current.syncStatus==='synced'&&String(note.updatedAt)>String(current.updatedAt||'')){await transaction('readwrite',store=>store.put(note));localMap.set(note.id,note);}}return true;}
  ...
  /** Lista anotacoes privadas depois de tentar incorporar a nuvem. */
  async function all(){requireUser();await ensureCloudSync();return localForCurrentUser();}
  ...
  /** Retorna anotacoes da conta atual dentro do raio informado. */
  ...
  /** Lista todas as anotacoes da conta atual pela ultima alteracao. */
