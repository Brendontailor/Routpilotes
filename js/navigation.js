...
function renderDetails() {
  ...
  const pointSource=p?.nameSourceUrl?` <a href="${esc(p.nameSourceUrl)}" target="_blank" rel="noopener noreferrer">Fonte do nome: ${esc(p.nameSource)}</a>`:'';
  const source=boundary?`<strong>${esc(boundary.classification||boundary.category)}</strong><br>${esc(boundary.description)} <a href="${esc(boundary.sourceUrl)}" target="_blank" rel="noopener noreferrer">Fonte: ${esc(boundary.source)}</a>`:p&&p.kind!=='referencia'?`<strong>${esc(p.classification||'Localidade')}</strong><br>${esc(p.description)}${pointSource}`:'Região de atendimento com limite operacional aproximado.';
...
function render() { renderLayout(); renderSearch(); renderNavigation(); renderDetails(); renderAreaInspector(); renderComparison();if(typeof renderRoutePlanner==='function')renderRoutePlanner(); renderContext(); updateLayers();if(typeof renderDesktopShell==='function')renderDesktopShell(); }
